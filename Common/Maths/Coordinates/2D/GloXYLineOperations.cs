...
public static class GloXYLineOperations
{
    ...
    /// </summary>
    public static bool DoesIntersect(GloXYLine l1, GloXYLine l2, double tol = 1e-9)
    {
    ...
    public static GloXYLine? ClipLineToRect(GloXYLine line, GloXYRect rect)
    {
        ...
        return new GloXYLine(new GloXYPoint(clippedX0, clippedY0), new GloXYPoint(clippedX1, clippedY1));
    }
    ...
    public static GloXYLine OffsetInward(GloXYLine line, GloXYPoint oppositePoint, double inset)
    {
        ...
        var toC = new GloXYPoint(oppositePoint.X - mid.X, oppositePoint.Y - mid.Y);
        ...
        var offset = new GloXYVector(nx * inset, ny * inset);
        return line.Offset(offset);
    }

    // Look for an intersection point between two lines.
    public static bool TryIntersect(GloXYLine l1, GloXYLine l2, out GloXYPoint result)
    {
        double d = (l1.P1.X - l1.P2.X) * (l2.P1.Y - l2.P2.Y) - (l1.P1.Y - l1.P2.Y) * (l2.P1.X - l2.P2.X);
        if (Math.Abs(d) < 1e-12)
        {
            result = GloXYPoint.Zero;
            return false;
        }
