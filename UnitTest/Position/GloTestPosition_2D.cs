...
public static partial class GloTestPosition
{
    private static void TestGloXYPoint(GloTestLog testLog)
    {
        ...
        testLog.AddResult("GloXYPoint Creation", pointA.X == 1 && pointA.Y == 2);

        double calcDistance = Math.Sqrt((4 - 1) * (4 - 1) + (5 - 2) * (5 - 2));
        testLog.AddResult("GloXYPoint Distance", GloValueUtils.EqualsWithinTolerance(pointA.DistanceTo(pointB), calcDistance, 0.001));
